...
ChecksumState* get_checksum(const std::string& dbc_name) {
  ...
  if (startswith(dbc_name, {"honda_", "acura_"})) {
    ...
    s = new ChecksumState({8, 8, 0, 0, true, MAZDA2019_CHECKSUM, &mazda2019_checksum});
  } else if (startswith(dbc_name, "mazda_2017")) {
    s = new ChecksumState({8, -1, 0, -1, true, MAZDA2017_CHECKSUM, &mazda2017_checksum});
  }
  return s;
}
