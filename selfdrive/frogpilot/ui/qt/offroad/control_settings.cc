...
FrogPilotControlsPanel::FrogPilotControlsPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> controlToggles {
    ...
    {"ConditionalExperimental", tr("Conditional Experimental Mode"), tr("Automatically switches to 'Experimental Mode' under predefined conditions."),
    ...
    {"SteerRatio", steerRatioStock != 0 ? QString(tr("Steer Ratio (Default: %1)")).arg(QString::number(steerRatioStock, 'f', 2)) : tr("Steer Ratio"),
                                                  tr("Use a custom steer ratio as opposed to comma's auto tune value."), ""},
  ...
  for (const auto &[param, title, desc, icon] : controlToggles) {
    ...
    QObject::connect(static_cast<FrogPilotParamManageControl*>(toggle), &ToggleControl::toggleFlipped, &updateFrogPilotToggles);
  ...
  std::set<QString> rebootKeys = {"AlwaysOnLateral", "AlwaysOnLateralMain", "NNFF", "NNFFLite"};
